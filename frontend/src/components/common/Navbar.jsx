...
const Navbar = () => {
  ...
  const { getCartCount } = useContext(cartDataContext);
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    // Update the 'search' param while keeping existing params (like category)
    ...
    // If user is not on products page, redirect them there with the search
      ...
      >
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-6">
          <div className="flex items-center justify-between h-16 lg:h-20 gap-4">
            ...
            {!isCartPage && (
            <div onSubmit={handleSearch} className="hidden md:flex grow max-w-2xl items-center">
              <div className="flex w-full border-2 border-blue-600 rounded-lg overflow-hidden">
                ...
                <button type="submit" className="bg-blue-600 px-6 py-2 text-white font-medium hover:bg-blue-700 transition-colors">
                  Search
                </button>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 lg:gap-6">
              <UserNavIcon/>
              <NavIcon icon={<ShoppingCart size={20} />} label="My cart" to="/cart" count={getCartCount()}/>
            </div>
          </div>
        ...
        {!isProductsPage &&(
          <div className="lg:hidden flex gap-2 overflow-x-auto px-4 py-2 no-scrollbar border-t border-gray-100 bg-white">
            {["All category", "Fashion", "Life Style", "Accessories", "Home"].map(
              (cat, i) => (
                <button
                  key={i}

...
const ProductCard = ({ product, view }) => {
  ...
  return (
   <div className={`bg-white border border-gray-200 rounded-lg hover:shadow-sm transition-shadow duration-200 overflow-hidden
      ...
      {/* Product Image */}
      <Link to={`/product/${product.id}`} className={`shrink-0 flex items-center justify-center 
      ...
      <div className="flex-1 flex flex-col justify-center">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-x-2 text-xs sm:text-sm text-gray-500">
          <div className="flex items-center">
            <RatingStars rating={product.rating} />
            <span className="ml-1 text-orange-400 font-medium">{product.rating}</span>
          </div>
          <span className="hidden sm:inline">•</span>
          <span className="hidden sm:inline">{product.orders} orders</span>
        </div>

        {product.freeShipping && (
          <div className="mt-1 text-xs sm:text-sm text-green-500 font-medium">
            Free Shipping
          </div>
        )}
